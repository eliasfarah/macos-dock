...
- **Two independent rules, not one.** The trailing row is one zone, not two: `[open and recent apps, in arrival order]`. (1) **Position never moves.** A slot is assigned once, when an unpinned app first appears, at the right end nearest the trash — and nothing afterwards changes it. Quitting keeps the slot (the app merely starts being remembered), reopening keeps it, quitting again keeps it. Running apps and recents interleave freely, so a closed app can sit to the left of an open one. (2) **Window membership is recency of *use*, decoupled from position.** The limit (default 3) counts **remembered entries — places the dock is holding — not apps that are currently shut** — and keeps whichever remembered apps were *closed most recently*, not whichever happen to sit rightmost. Closing an app refreshes its recency every time, first close or fifth, which is what lets an old, aged-out recent come back into view the moment it's closed again — without moving it, since that's still governed by rule (1) alone. An app stays in the limit's sum while it is open, which is what stops reopening a recent from freeing a place and summoning an unrelated app that aged out long ago; a running app is still never dropped, so one past the window (or never remembered at all) is drawn because it is open. Recency is session-only (seeded from arrival order at each `load()`), so a fresh boot is still fully reproducible — it only diverges once something is quit again during the session. The invariant to test against: launching or quitting an app adds or removes **that app's own icon and no other**, and closing an app is never a no-op for its place in the window even if it was already remembered. `visibleIds()` returns the whole row in display order; focus is not an input, close *order* is the whole point.
