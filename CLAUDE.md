...
- **An icon never moves.** The trailing row is one zone, not two: `[open and recent apps, in arrival order]`. A slot is assigned once, when an unpinned app first appears, at the right end nearest the trash — and nothing afterwards changes it. Quitting keeps the slot (the app merely starts being remembered), reopening keeps it, quitting again keeps it. Running apps and recents interleave freely, so a closed app can sit to the left of an open one. The limit (default 3) counts **remembered entries — places the dock is holding — not apps that are currently shut.** An app stays in the sum while it is open, which is what stops reopening a recent from freeing a place and summoning an app that aged out long ago; a running app is still never dropped, so one past the window (or never remembered at all) is drawn because it is open. The queue is FIFO **by slot age**: past the limit the oldest slot drops out, even if its app is the one most recently used. The invariant to test against: launching or quitting an app adds or removes **that app's own icon and no other**. `visibleIds()` returns the whole row in display order; focus and close time are not inputs at all.
