...
- **The row is ordered by recency of use; an icon on screen does not jump while you're looking at it.** The trailing row is one zone, not two: `[open and recent apps, most recently used last]`. An app that is *not currently shown* — brand new, or a recent that aged out of the visible window — takes the slot nearest the trash the moment it becomes relevant again: launching it or closing it both count, every time, first use or fifth. An app that *is already shown* — running, or a recent still inside the window — does not move just because you look at it or relaunch it; only a genuine quit relocates it. This is what stops the two failure modes that got fixed here in turn: launching pushing recents off the end (fixed by not letting launch touch the queue for anything already visible), and an already-visible icon sliding sideways the instant you click it (fixed by the same rule, the other direction). The limit (default 3) counts **remembered entries — places the dock is holding — not apps that are currently shut**: an app stays in the sum while it is open, which is what stops reopening a recent from freeing a place and summoning an unrelated app that aged out long ago; a running app is still never dropped, so one past the window (or never remembered at all) is drawn because it is open. `visibleIds()` returns the whole row in display order; focus is not an input, close *and reopen* order is the whole point. `_promote()` is the one primitive that relocates an id — to the end, always — and `_adopt()`/`_retire()` are just "promote if not already visible" and "promote, unconditionally" respectively.
